...
- [x] Add tests for the new dependency and the migrated routers:
  ...
  **Implementation notes:**
  - **`backend/tests/test_active_membership_dep.py`** (new file, 11 tests). Mirrors the unit-style approach used in `test_rbac.py` — calls `get_active_membership(...)` directly with kwargs (no FastAPI client) so the dep's branching logic is exercised in isolation. Covers all 5 task-list scenarios plus 3 follow-on guards: header-missing-multi-memberships → 400, header-missing-zero-memberships → 400, and superadmin-with-real-membership returns the persisted row (not a synthesized one) so audit data stays accurate. A nested `TestRequireMembershipRoles` class covers the role-gate factory: matching role passes, non-matching 403s, synthesized superadmin passes when `superadmin` is in the allow-list.
  - **`backend/tests/test_funds_api.py`** — added `TestCrossOrgScopingViaHeader` (4 tests) using a multi-org admin (memberships in Org A *and* Org B). Asserts: (1) no header → 400 `"X-Organization-Id required"`, (2) `X-Organization-Id: org_a` lists only Fund A and `X-Organization-Id: org_b` lists only Fund B, (3) `GET /funds/{fund_b}` with `X-Organization-Id: org_a` → 403 (no cross-org leakage on the detail route — `repo.membership_can_view` rejects the mismatched org), (4) a single-org admin spoofing a different org via the header → 403 from the dep itself, never reaches the router body. Uses a per-class `_seed_multi_org_admin` helper since the file's existing `_seed_user` only creates one membership.
