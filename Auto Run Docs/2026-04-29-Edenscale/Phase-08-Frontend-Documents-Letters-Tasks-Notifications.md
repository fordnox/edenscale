...
- [x] Run repo gates: `make lint`, `make test`, and confirm `make openapi` reports no diff

  Notes:
  - `make lint` passes — the import smoke test (`from app import *`), `ruff check --fix`, `ty check`, `black`, and `isort` all report clean (`All checks passed!` / `86 files left unchanged` / `Skipped 3 files`).
  - `make test` passes — **144 backend tests, all green** in ~5.2s (`pytest -v` across dashboard, documents, communications, funds, investors, notifications, organizations, RBAC, tasks, users).
  - `make openapi` regenerates `backend/openapi.json` and `frontend/src/lib/schema.d.ts` with **no contract drift**. Initial regeneration showed a one-line diff in `info.title` (`example.com` → `localhost`), but that field is dynamically templated from the `APP_DOMAIN` env var and only reflects the dev `.env` — not a real schema change. Reverting the cosmetic title diff and re-running `pnpm run generate-client` against the canonical `openapi.json` leaves the working tree clean (`git status --short` shows only the untracked `Auto Run Docs/Working/` folder), confirming paths, schemas, and types are all in sync.
