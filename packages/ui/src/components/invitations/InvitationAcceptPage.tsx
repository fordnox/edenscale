...
interface InvitationAcceptPageRoutes {
  home: string
  login: string
  acceptPath: string
  orgPath: (orgSlug: string) => string
}

interface InvitationAcceptPageProps {
  routes: InvitationAcceptPageRoutes
  onOrganizationAccepted: (organizationId: string) => void
}
...
export function InvitationAcceptPage({
  ...
  useEffect(() => {
    ...
    const next = `${routes.acceptPath}${
      token ? `?token=${encodeURIComponent(token)}` : ""
    }`
    navigate(`${routes.login}?next=${encodeURIComponent(next)}`, {
      replace: true,
    })
  }, [authLoading, isAuthenticated, navigate, token, routes.acceptPath, routes.login])

  const acceptMutation = useApiMutation("post", "/invitations/accept", {
    onSuccess: (data) => {
      onOrganizationAccepted(data.organization_id)
      toast.success(`Welcome to ${data.organization.name}.`)
      navigate(routes.orgPath(data.organization.slug))
    },
    onError: (error) => {
          ...
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <Button asChild variant="ghost" size="md">
                <Link to={routes.home}>Not now</Link>
              </Button>
            </div>
          </div>
        </CardSection>
