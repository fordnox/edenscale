...
const cmd =
  "pnpm turbo run build --filter=web --filter=manager --filter=investor --filter=superadmin"
...
console.log(
  "assembled gateway dist <- web(/) + manager(/manager) + investor(/investor) + superadmin(/superadmin)",
)
