...
class TestUploadSizeEnforcement:
    ...
    def test_oversized_declared_length_rejected_before_buffering(
        ...
        assert (
            storage_module.get_storage().read("documents/tok/oversized.bin") is None
        )
    ...
    def test_stream_exceeding_cap_is_aborted_mid_transfer(
        ...
        # catch this.
        monkeypatch.setattr(
            "app.routers.documents._MAX_UPLOAD_BYTES", 10, raising=True
        )
        _seed_user("hanko-any", UserRole.lp, email="any@example.com")
        override_user("hanko-any")
        ...
        assert resp.status_code == 413
        assert (
            storage_module.get_storage().read("documents/tok/streamed.bin") is None
        )
