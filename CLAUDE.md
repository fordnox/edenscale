...
- **Config**: `app/core/config.py` exposes a single `settings` instance (`pydantic-settings`, reads `.env`). Key env vars: `APP_DOMAIN`, `APP_DATABASE_DSN`, `REDIS_URL`, `HANKO_API_URL`, `OPENROUTER_API_KEY`, `SUPERADMIN_EMAIL` (comma-separated; superadmins are config-defined via `User.is_superadmin`, never stored in the database — users have no global role column, per-org roles live on membership rows).
