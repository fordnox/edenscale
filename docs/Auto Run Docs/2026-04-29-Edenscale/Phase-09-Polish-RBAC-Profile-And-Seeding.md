...
- [x] Capture architecture knowledge as structured Markdown for the project's docs graph:
  ...
  **Implementation notes (Iteration 00001, 2026-04-30):**
  - Created the new `docs/` tree with `architecture/` (5 reference docs) and `decisions/` (2 ADRs). All seven files carry the requested YAML front matter (`type`, `title`, `created: 2026-04-29`, `tags`) and use `[[Wiki-Link]]` cross-references in a `related:` array plus inline body links. The link graph is bidirectional: every architecture doc links to `[[System-Overview]]` and at least one sibling, both ADRs link back from `system-overview.md`, and `rbac-model.md` ↔ `database-schema.md` ↔ `api-layering.md` form a triangle so the DocGraph viewer renders a connected component.
  - `system-overview.md` has the topology block (SPA → Hanko, SPA → FastAPI → Postgres, FastAPI → Redis/arq) and a "where to look next" routing table — it's intended as the entry point for new contributors. `api-layering.md` walks the router → repository → model → schema flow with the actual `list_funds` example pulled from `app/routers/funds.py` + `app/repositories/fund_repository.py`. `database-schema.md` summarises the four entity groups (Identity & Tenancy, Funds & Commitments, Capital Flows, Supporting) with a textual ER diagram, calls out the `(communication_id, user_id)` uniqueness constraint that bit the seed script, and lists every enum + the audit fan-out shape. `rbac-model.md` includes the dependency stack (`HTTPBearer → get_current_user → get_current_user_record → require_roles`), the per-entity visibility matrix for the three roles, and the LP join template (`investor_contacts → investors → commitments → fund`). `frontend-routing.md` lists every route in `App.tsx` (currently 14, including `/login`) with layout + sidebar visibility + page-level role gate columns, plus the `useNavItems` role mapping.
